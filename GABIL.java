...
//            ArrayList<int[]> padres = tournamentSelection(2, population, Pr);
    ...
    private void mutate(ArrayList<int[]> Ps) {
        ...
        System.out.println("Número de mutaciones, m = "+n);
        Random gen = new Random();
        int indexC;
        int whichBit;
        int[] candidate;
    ...
    private void addAlternative(ArrayList<int[]> Ps, double prob) {
        Random gen = new Random();
        int howMany = (int) (prob*Ps.size());
        int which;
        int whichCondition;
        int[] hypToChange;
        for(int i = 0; i < howMany; i++) {
            which = (int) (gen.nextDouble()*(Ps.size()));
            hypToChange = Ps.get(which);
            while (true) {
                whichCondition = (int) (gen.nextDouble()*(hypToChange.length));
                if (whichCondition != 0 && (whichCondition % 35 == 0)) {
                    continue;
                }
                else {
                    if (hypToChange[whichCondition] == 0) {
                        hypToChange[whichCondition] = 1;
                        break;
                    } else
                        continue;
                }
    ...
    private void dropCondition(ArrayList<int[]> Ps, double prob) {
        Random gen = new Random();
        int howMany = (int) (prob*Ps.size());
        int which;
        int whichCond;
        ...
        for(int w = 0; w < howMany; w++) {
            base = 0;
            whichCond = (int) (gen.nextDouble()*(10));
            for(int i = 0; i < whichCond; i++) {
                base += Parser.seq[i];
            }
            which = (int) (gen.nextDouble()*(Ps.size()));
            hypToChange = Ps.get(which);
            nrules = hypToChange.length / Parser.REP_SIZE;
            whichRule = (int) (gen.nextDouble()*(nrules));
...
//                <= 6.0) {
            ...
            mutate(Ps);
//            addAlternative(Ps, addProb);
//            dropCondition(Ps, dropProb);
            population = Ps;
